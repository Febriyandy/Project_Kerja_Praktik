...
export const exportToPDF = (data, { tahunPelajaran }) => {
    ...
    doc.setFontSize(12);
    doc.text(
        `Laporan Infaq Siswa Perguruan Muhammadiyah Tanjungpinang \nTahun Pelajaran ${tahunPelajaran}`,
        pageWidth / 2, 
        20, 
        { align: 'center' }
    );
    ...
    // Table Header
    const tableColumn = ["NO", "TANGGAL", "KETERANGAN", "SATUAN", "DEBIT", "KREDIT", "SALDO"];
    const tableRows = [];
    ...
    doc.autoTable({
        ...
        columnStyles: {
            ...
            1: { cellWidth: 30, halign: 'center', valign: 'middle' }, // TANGGAL
            2: { cellWidth: 60, halign: 'center', valign: 'middle' }, // KETERANGAN
            3: { cellWidth: 50, halign: 'center', valign: 'middle' }, // SATUAN
    ...
    // Save the PDF
    doc.save(`Laporan_Infaq_${tahunPelajaran}.pdf`);
};
