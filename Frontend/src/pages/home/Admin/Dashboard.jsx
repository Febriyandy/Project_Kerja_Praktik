...
import GrafikDanaBos from "./GrafikDanaBos";
import GrafikDanaSPP from "./GrafikDanaSPP";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (data_kasSekolahSD.length > 0 || data_kasSekolahSMP.length > 0 || data_kasSekolahSMA.length > 0 || data_donatur.length > 0) {
  ...
  const getDataKasSekolahSD = async () => {
    try {
      const response = await axiosJWT.get(`${import.meta.env.VITE_API_URL}/DataKasSekolah/ByNPSN/11001970`, {
        headers: {
  ...
  const getDataKasSekolahSMP = async () => {
    try {
      const response = await axiosJWT.get(`${import.meta.env.VITE_API_URL}/DataKasSekolah/ByNPSN/11001860`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const getDataKasSekolahSMA = async () => {
    try {
      const response = await axiosJWT.get(`${import.meta.env.VITE_API_URL}/DataKasSekolah/ByNPSN/11001974`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      <section className="flex pr-3">
        ...
        <section className="mt-16 w-11/12 mx-auto h-full">
          ...
          <div className="w-full bg-accent mt-5 bg-opacity-10 rounded-lg border-2 border-accent mx-auto grid grid-cols-4 gap-5 p-5">
            ...
            <div className="h-36 p-3 bg-white shadow-md rounded-md">
              <div className="flex justify-between items-center">
                <p className="font-medium text-xs font-body text-gray-400  py-2">
                  Laporan Keuangan <br /> Total Saldo
                </p>
                <div className="w-14 h-12 flex items-center bg-[#00A9DC] bg-opacity-10 rounded-md">
                  <MdOutlinePayments className="mx-auto text-3xl text-[#00A9DC]" />
                </div>
              </div>
              <p className="font-body  font-bold text-xl">{formatCurrency(totalKasYayasan)}</p>
              <Link
            ...
            {data_donatur.map((data, index) => (
            <div className="h-36 p-3 bg-white shadow-md rounded-md">
              <div  key={index} className="flex justify-between items-center">
                <p className="font-medium text-xs font-body text-gray-400  ">
                  Arus Kas Yayasan<br />Tahun Pelajaran {data.tahun_pelajaran}
                </p>
                <div className="w-14 h-12 flex items-center bg-[#00A9DC] bg-opacity-10 rounded-md">
                  <FaHandHoldingHeart className="mx-auto text-3xl text-[#00A9DC]" />
                </div>
              ...
              <p className="font-body mt-1 font-bold text-xl">{formatCurrency(data.latestSaldo)}</p>
              <Link
                to={`/admin/donatur/detail/${data.id}`}
                className="py-2 px-6 bg-accent mt-2 hover:bg-primary float-left text-white font-body text-xs rounded-md font-medium"
              >
                Lihat Selengkapnya
              </Link>
            ...
            {data_kasSekolahSD.map((data, index) => (
            <div className="h-36 p-3 bg-white shadow-md rounded-md">
              <div  key={index} className="flex justify-between items-center">
                <p className="font-medium text-xs font-body text-gray-400  ">
                  Arus Kas SD <br />Tahun Pelajaran {data.tahun_pelajaran}
                </p>
                <div className="w-14 h-12 flex items-center bg-[#00A9DC] bg-opacity-10 rounded-md">
              ...
              <p className="font-body mt-1 font-bold text-xl">{formatCurrency(data.latestSaldo)}</p>
              <Link
                to={`/admin/arus_kas/11001970/detail/${data.id}`}
                className="py-2 px-6 bg-accent mt-2 hover:bg-primary float-left text-white font-body text-xs rounded-md font-medium"
              >
                Lihat Selengkapnya
              </Link>
            ...
            {data_kasSekolahSMP.map((data, index) => (
            <div className="h-36 p-3 bg-white shadow-md rounded-md">
              <div  key={index} className="flex justify-between items-center">
                <p className="font-medium text-xs font-body text-gray-400  ">
                  Arus Kas SMP <br />Tahun Pelajaran {data.tahun_pelajaran}
                </p>
                <div className="w-14 h-12 flex items-center bg-[#00A9DC] bg-opacity-10 rounded-md">
                  <MdOutlineAccountBalanceWallet className="mx-auto text-3xl text-[#00A9DC]" />
                </div>
              ...
              <p className="font-body mt-1 font-bold text-xl">{formatCurrency(data.latestSaldo)}</p>
              <Link
                to={`/admin/arus_kas/11001860/detail/${data.id}`}
                className="py-2 px-6 bg-accent mt-2 hover:bg-primary float-left text-white font-body text-xs rounded-md font-medium"
              >
                Lihat Selengkapnya
              </Link>
            ...
            {data_kasSekolahSMA.map((data, index) => (
            <div className="h-36 p-3 bg-white shadow-md rounded-md">
              <div  key={index} className="flex justify-between items-center">
                <p className="font-medium text-xs font-body text-gray-400  ">
                  Arus Kas SMA <br />Tahun Pelajaran {data.tahun_pelajaran}
                </p>
                <div className="w-14 h-12 flex items-center bg-[#00A9DC] bg-opacity-10 rounded-md">
                  <MdOutlineAccountBalanceWallet className="mx-auto text-3xl text-[#00A9DC]" />
                </div>
              ...
              <p className="font-body mt-1 font-bold text-xl">{formatCurrency(data.latestSaldo)}</p>
              <Link
                to={`/admin/arus_kas/11001974/detail/${data.id}`}
                className="py-2 px-6 bg-accent mt-2 hover:bg-primary float-left text-white font-body text-xs rounded-md font-medium"
              >
                Lihat Selengkapnya
              </Link>
