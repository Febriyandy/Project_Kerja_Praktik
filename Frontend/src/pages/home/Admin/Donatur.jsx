...
const Donatur = () => {
  ...
  const [data_DonaturYayasan, setData_DonaturYayasan] = useState([]);
  ...
  const getDataDonaturYayasan = async () => {
    try {
      ...
      setData_DonaturYayasan(sortedData);
    } catch (error) {
  ...
  const saveData = async (e) => {
    ...
    try {
      ...
      await axiosJWT.post(`${import.meta.env.VITE_API_URL}/TambahDonaturYayasan`, formData, {
        headers: {
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil Membuat Donatur Periode Baru!",
      ...
      <section>
        ...
        <section className="mt-16 w-11/12 mx-auto h-full">
          <div className="flex justify-between">
            <p className="font-body mt-3 text-base font-bold text-secondary ">Arus Kas Yayasan Majelis Dikdasmen dan PNF Muhammadiyah Tanjungpinang</p>
            <button
          ...
          <div className="mt-5 grid grid-cols-4 gap-5">
            {data_DonaturYayasan.map((data, index) => (
