...
const RiwayatSPPBendahara = () => {
  ...
  useEffect(() => {
    if (npsn) {
      getDataSiswa(); // Ambil data siswa hanya ketika npsn berubah
    }
  }, [npsn]);
  ...
  useEffect(() => {
    if (npsn) {
      ...
      setSelectedKelas(""); // Reset selected kelas ketika opsi kelas berubah
    }
  }, [npsn]);
  ...
  const getDataSiswa = async () => {
    try {
      const response = await axiosJWT.get(
        `${import.meta.env.VITE_API_URL}/DataSPPSekolah/${npsn}`,
        {
          headers: {
  ...
  // Filter data based on selected tahun pelajaran and kelas
      ...
      <section className="flex pr-3">
        ...
        <section className="min-w-0 h-full mt-16">
          ...
          <div className="flex gap-4">
              <Link to="/bendahara" className="text-2xl text-secondary">
                <IoArrowBackCircleOutline />
              </Link>
              <p className="font-body text-base font-bold text-secondary">
                Riwayat Pembayaran SPP {nama_sekolah}
              </p>
            </div>
          ...
          <div className=" mx-auto mb-10 mt-5 px-5 h-full border border-gray-300 shadow-md rounded-lg overflow-y-auto">
...
export default RiwayatSPPBendahara;
